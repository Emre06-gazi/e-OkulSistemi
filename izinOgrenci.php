...
<head>
    ...
    <title>Öğrenci İzin İşlemleri</title>
    <style>
        body {
...
<button class="back-button" onclick="goBack()">←</button>
    ...
    ?>
    <h1>Öğrenci İzin İşlemleri</h1>

    <!-- İzin Talep Formu -->
    <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post">
        <label for="kullanici_id">Öğrenci Seç:</label>
        <select name="kullanici_id" required>
            <?php
            // Kullanıcı tablosundan öğrencileri seç
            $selectUsersSql = "SELECT id, kullanici_adi FROM kullanici WHERE rol = '2'";
            ...
            if ($result->num_rows > 0) {
                ...
                echo "<option value='' disabled selected>Öğrenci bulunamadı</option>";
            }
            ?>
        </select>
